...
[Git 规范](/engineering/git.md) 已经对 `Git` 提交日志的格式进行了约束，为何还要再约束 `Changelog` 的格式呢？
...
- 2.6.【推荐】更新类型 `type` 与 Git message header 中的 [`type`](/engineering/git.md#1.3.1-type) 相关联，可以不一一对应
