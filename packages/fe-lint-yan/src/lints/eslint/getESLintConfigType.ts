 ...
 * @param cwd
 * @param pkg
 * @returns eslint-config-yan/index
 * @returns eslint-config-yan/react
 * @returns eslint-config-yan/typescript/index
 * @returns eslint-config-yan/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'eslint-config-yan/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
